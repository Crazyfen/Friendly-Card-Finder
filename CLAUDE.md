...
1. **Card search** (default text handler): `RefreshStaleUserLists` refreshes users older than `CARD_LIST_REFRESH_HOURS` (default 3h) via 5-worker pool → `SearchCard` → format → split/send (4096-byte Telegram limit; `splitMessage` cuts on newlines, respects UTF-8 runes). A query wrapped in quotes (`"Shock"`; also `''`/`«»`/`“”`/`‘’` and low-9 `„“`/`„”` — see `exactQuotePairs`) is an exact-name match: `ParseExactQuery` strips the quotes and sets the `exact` flag passed through `DeckboxSaver.SearchCard`. `SearchCards` (multi-line) parses each line once up front, so `SearchQueries`/`NotFound` echo the stripped names
